...
try {
    ...
    // Truncate first to make it clean
    $db->exec("SET FOREIGN_KEY_CHECKS = 0;");
    $db->exec("TRUNCATE TABLE employee_data_correction_requests;");
    ...
    $db->exec("SET FOREIGN_KEY_CHECKS = 0;");
    $db->exec("TRUNCATE TABLE employee_reimbursement_claims;");
    $db->exec("SET FOREIGN_KEY_CHECKS = 1;");
    ...
    $db->exec("SET FOREIGN_KEY_CHECKS = 0;");
    $db->exec("TRUNCATE TABLE employee_leave_requests;");
    $db->exec("SET FOREIGN_KEY_CHECKS = 1;");
    ...
    $db->exec("DELETE FROM employee_attendance WHERE attendance_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)");
    ...
    ");
    echo "Table hr_settings created or verified.\n";

    // 15. Seed default HR settings
    $defaultSettings = [
        // --- Attendance / Location ---
    ...
    $stmtSetting = $db->prepare("
        INSERT INTO hr_settings (`key`, `value`, `label`, `group`)
