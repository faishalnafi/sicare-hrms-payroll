...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div class="space-y-1">
            ...
            <p class="text-on-surface-variant font-medium text-sm">Konfigurasi lokasi kantor, jam kerja, toleransi, dan kebijakan WFA/WFC/WFO.</p>
        </div>
...
if (typeof window.currentPickerMapType === 'undefined') {
    window.currentPickerMapType = 'roadmap'; // persists within session
}
        ...
        document.getElementById('checkoutPreview').innerHTML = `
            <div class="flex items-start gap-2">
                ...
                <div>
                    ...
                    ${minEndDesc}Karyawan pulang hingga <strong>${coDeadlineStr}</strong> masih dianggap <span class="text-emerald-700 font-semibold">Tepat Waktu</span>. Pulang setelah <strong>${coDeadlineStr}</strong> tercatat sebagai <span class="text-orange-700 font-semibold">Pulang Terlambat</span>.
                </div>
            </div>
        `;
    }
...
function editHoliday(id, currentDate, currentDesc) {
    Swal.fire({
        ...
        fd.append('holiday_date', result.value.date);
        fd.append('description', result.value.desc);
        const endpoint = window.location.pathname.startsWith('/hrops') ? '/hrops/holidays/update' : '/admin/holidays/update';
